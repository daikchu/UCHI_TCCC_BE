...
public class Crypter {
    ...
    /*-----Random key-----*/
    private static final String RANDOM_KEY = "UCHI_STP";

    public static String getHashPassword(String passWord) {
        String salt = getHashe(RANDOM_KEY);
        String hash = "";

        for (int i = 0; i < STRETCH_COUNT; i++) {
            hash = getHashe(hash + passWord + salt);
        }
        return hash;
    }

    private static String getHashe(String value) {
        MessageDigest md = null;
        StringBuffer sb = new StringBuffer();
        try {
            md = MessageDigest.getInstance(ALGORITHM);
        } catch (NoSuchAlgorithmException e) {
            throw new SystemException("Thuật toán là không đúng.", e);
        }
